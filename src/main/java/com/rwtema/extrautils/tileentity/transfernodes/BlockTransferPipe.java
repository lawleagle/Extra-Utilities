...
public class BlockTransferPipe extends BlockMultiBlockSelection implements ICreativeTabSorting, IPipeBlock {
  ...
  public BlockTransferPipe(int pipePage) {
    ...
    setBlockName("extrautils:pipes" + ((pipePage == 0) ? "" : ("." + pipePage)));
    setBlockTextureName("extrautils:pipes");
  ...
  public BoxModel getWorldModel(IBlockAccess world, int x, int y, int z) {
    return getPipeModel(world, x, y, z, (IPipe)null);
  }
  ...
  public BoxModel getInventoryModel(int metadata) {
    ...
    if (pipe != null) {
      ...
      if (((IPipeCosmetic)getPipe(metadata)).baseSize() > 0.125F)
        boxes.add((new Box(0.5F - ((IPipeCosmetic)pipe).baseSize(), 0.5F - ((IPipeCosmetic)pipe).baseSize(), 0.5F - ((IPipeCosmetic)pipe).baseSize(), 0.5F + ((IPipeCosmetic)pipe).baseSize(), 0.5F + ((IPipeCosmetic)pipe).baseSize(), 0.5F + ((IPipeCosmetic)pipe).baseSize())).setTexture(((IPipeCosmetic)getPipe(metadata)).baseTexture()).setLabel("base")); 
    } 
    return boxes;
  }
  ...
  public void breakBlock(World par1World, int par2, int par3, int par4, Block par5, int par6) {
    ...
    if (par1World.getBlock(par2, par3, par4) != FMPBase.getFMPBlockId() && 
      tile instanceof TileEntityFilterPipe) {
...
/* Location:              C:\Users\Emanuel\src\` decompile\mods\extrautilities-1.2.12-deobf.jar!\com\rwtema\extrautils\tileentity\transfernodes\BlockTransferPipe.class
